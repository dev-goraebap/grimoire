...
description: >
  ...
  큐레이트한 **팀 공개 지침(public rules) 초안**을 생성한다. 두 가지 트랙
  지원: A) Multi-agent → 루트 `AGENTS.md` + Claude/Gemini 브릿지,
  B) Claude-only → 루트 `CLAUDE.md` 자체를 공개 지침으로 사용(브릿지 불필요).
  트랙은 사용자가 명시 선택(자동 추측 금지). Draft-first: 분석 → 완성된 초안
  한 번에 제시 → 사용자 비판으로 iterate. 신호가 빈약하면 "뭘 만들려고
  하세요?" 한 가지만 묻는다. 생성된 파일은 "초안"이며, 후속 스킬
  (Boundaries 누적, 스킬/MCP 정리 등)로 계속 다듬는 것을 전제로 한다.
  ...
  "AGENTS.md 만들어줘", "AGENTS.md 초안", "CLAUDE.md 만들어줘",
  "에이전트 규칙 생성", "프로젝트 규칙 설정", "공개 지침 만들어줘",
  "팀 규칙 초안", "팀 공유 규칙".
  새 프로젝트를 시작하거나, AI 에이전트를 온보딩하거나, 팀 공개 지침을
  표준화할 때 사용.
metadata:
  author: dev-goraebap
  version: "0.2.0"
